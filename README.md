...
- 입력·화면 전환·빠른 이동·전진·재시작·재시작 후 화면을 목적별로 묶은 GUI 타이밍 설정
- 겹친 초록 차량 아이콘 군집과 지도 차량 카드의 해상도 정규화 판독
- 세 차량 아이콘 자동 클릭·OCR, 목표 발견 경고음, 비목표 시 ENG→KOR 1회 재시작
...
1. GUI를 실행합니다.
2. GUI에서 타이밍 값을 PC 환경에 맞게 조정합니다. 조향 키는 `A`, `D`, `사용 안 함` 중
   선택하고 조향 시간은 ms 단위로 별도 설정합니다. 재시작 후 첫 Enter 대기는
   기본 15초, 두 번째 Enter 뒤 오픈월드 대기는 기본 30초입니다. 클라이언트 재시작은
   기본 60초이며, 오픈월드 진입 후 `M`까지의 별도 대기는 기본 10초입니다.
   나머지 기본값은 입력 850ms, 화면 전환 2.5초, 빠른 이동 15초, 전진 5초입니다.
3. FH6를 KOR 또는 ENG 오픈월드 정차 상태로 두고 전경에서 F1을 누릅니다.
4. 화면 전환 설정에서 파생된 1~2초 준비 대기 후 GUI가 `M`으로 월드 맵에 들어갑니다.
   시작할 때는 언어를 판별하지 않습니다.
5. Evolving World 필터에서 지구본을 찾아 빠른 이동한 뒤 GUI의 `전진 시간`만큼
   `W`를 유지합니다. GUI에서 선택한 조향 키를 설정 시간만큼 함께 눌러 아이콘 군집에서
   비켜납니다. 기본값은 `D` 500ms입니다.
6. 다시 `M`으로 지도를 열고 맨 위 `All` 기준으로
   `PageDown → Enter ×2 → Down ×26 → Enter`를 실행합니다.
7. 지도를 다시 열어 최대 확대하고 초록 Aftermarket 차량 아이콘 군집을 확인합니다.
8. 목표가 없으면 언어를 바꿔 재시작한 뒤 `Enter → 대기 → Enter → 대기 → Esc → 짧은 대기 → M`으로
   오픈월드에 복귀하고 다음 회차를 시작합니다. 별도의 FH6 포커스 대기 단계는 없습니다.
9. 언제든 F2로 세션을 끝냅니다. 전진 중에도 W, A, D가 즉시 해제됩니다.
...
정확한 창 제목과 16:9 화면을 확인하고, 지구본 빠른 이동과 설정된 시간만큼의 전진을 거쳐
